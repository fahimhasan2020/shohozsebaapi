<template>
    <div>
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-dark" style="overflow-x: scroll">
                        ...
                        <tr>
                            ...
                            <th scope="col" style="vertical-align: middle">Phone</th>
                            <th scope="col" style="width: 150px;vertical-align: middle">Name</th>
                            <th scope="col" style="width: 150px;vertical-align: middle">Total</th>
                            <th scope="col" style="width: 150px;vertical-align: middle">Status</th>
                        ...
                        <tr v-for="permission in permissionOutput.data" :key="permission.id">
                            ...
                            <td style="vertical-align: middle">{{permission.user_name}}</td>
                            <td style="vertical-align: middle">{{permission.full_total}}</td>

                            <td style="vertical-align: middle">
                                <select name="status" @change="handleChangeDelivery($event,permission.id)" id="status" :value="permission.status">
                            ...
                            <td style="vertical-align: middle">
                                ...
                                &nbsp;<a href="#" class="text-warning" data-toggle="tooltip" data-placement="top" title="Edit info"><i class="fa fa-eye" data-toggle="modal" data-target="#exampleModals" @click.prevent="permissionPopUp(permission)"></i></a>
                            </td>
                        </tr>
                        </tbody>
                    </table>
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabels">Order Details</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <div v-if="singleDate !== null" class="modal-body">
                        <p>User name: {{ singleDate.name }}</p>
                        <p>Email: {{ singleDate.email }}</p>
                        <p>phone: {{ singleDate.phone }}</p>
                        <p>Address: {{ singleDate.address }}</p>
                        <p>Total: {{ singleDate.total }}</p>
                        <p>Vat: {{ singleDate.vat }}</p>
                        <p>Full Total: {{ singleDate.full_total }}</p>
        ...
        <tr>
            ...
            <th style="border: 1px solid #ccc; padding: 8px; text-align: center;">Total Units</th>
            <th style="border: 1px solid #ccc; padding: 8px; text-align: center;">Total Unit Price</th>
        </tr>
       <tr v-for="item in JSON.parse(singleDate.cart)" :key="item.medicine_id">
                <td style="border: 1px solid #ccc; padding: 8px; text-align: center;">
                    <img :src="item.thumb" :alt="item.medicine_name" style="max-width: 50px; max-height: 50px;">
                </td>
                <td style="border: 1px solid #ccc; padding: 8px; text-align: center;">{{ item.medicine_name }}</td>
                <td style="border: 1px solid #ccc; padding: 8px; text-align: center;">{{ item.total_unit * 3 }}</td> <!-- Assuming 3 is a constant multiplier -->
                <td style="border: 1px solid #ccc; padding: 8px; text-align: center;">{{ item.total_unit_price }} BDT</td>
            </tr>
    </table>
...
<script>
    export default {
        ...
        methods: {
            ...
            handleChangeDelivery(event,id){
                this.$inertia.put('/admins/edit/medicine/delivery/',{id:id,value:event.target.value});
            },
            dataTable(){
                this.isLoading = true;
                //this.$loading(true);
